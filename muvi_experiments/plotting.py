...
def group(model, factor_idx, group_idx, groupby, groups=None, pl_type=HEATMAP, **kwargs):
    ...
    factor_adata = factor_adata[group_idx].copy()
...
def rank(model, groupby, group_idx, n_factors=10, pl_type=None, sep_groups=True, **kwargs):
    factor_adata = model.get_factors("anndata")
    factor_adata = factor_adata[group_idx].copy()
...
def scatter_rank(model, groupby, group_idx, groups=None, **kwargs):
    factor_adata = model.get_factors("anndata")
    factor_adata = factor_adata[group_idx].copy()
    sc.tl.rank_genes_groups(factor_adata, groupby)
...
def groupplot_rank(model, groupby, group_idx, groups=None, pl_type=STRIPPLOT, top=1, **kwargs):
    factor_adata = model.get_factors("anndata")
    factor_adata = factor_adata[group_idx].copy()
    sc.tl.rank_genes_groups(factor_adata, groupby)
    group_df = sc.get.rank_genes_groups_df(factor_adata, group=groups)
    ...
    if model.model_opts.nonnegative_factors[group_idx]:
        scores_key = "scores"
