...
from matplotlib import colors as mcolors  # noqa: F401 - exported for notebook/script reuse.
...
def set_effect_colorbar_ticks(fig, vmin, vmax, include_zero=True):
    ...
    for ax in fig.axes:
        ...
        if "|effect size|" in f"{title} {xlabel} {ylabel}".lower():
            continue
        is_effect_cbar = title == "Effect size" or xlabel == "Effect size" or ylabel == "Effect size"
