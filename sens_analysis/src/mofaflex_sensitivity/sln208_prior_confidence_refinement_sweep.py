...
if __package__ in {None, ""}:
    ...
    from mofaflex_sensitivity.sln208_prior_noise_refinement import DEFAULT_DATA_PATH, run
    from mofaflex_sensitivity.sln208_prior_noise_refinement_plotting import (
        aggregate_with_errorbars,
        save_metric_grid_plot,
    )
else:
    from .sln208_prior_noise_refinement import DEFAULT_DATA_PATH, run
...
DEFAULT_OUT_DIR = Path("artifacts/sln208_prior_confidence_refinement_sweep")
DEFAULT_CONFIDENCE_LEVELS = [0.9, 0.95, 0.97, 0.99, 0.995, 0.999]
SUMMARY_PANELS = [
    ("mean_average_precision_true", "Mean AP vs true prior", None),
    ("mean_average_precision_noisy", "Mean AP vs reference prior", None),
...
FOCUS_SOURCE_PANELS = [
    ...
    ("average_precision_noisy", "mean AP vs reference prior", None),
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Sweep prior annotation confidence on SLN208 using the clean Hallmark+MCA prior, "
            "train MOFA-FLEX, and summarize refinement robustness across seeds."
        )
    )
...
def _slugify_confidence_level(value: float) -> str:
    text = f"{value:.3f}".rstrip("0").rstrip(".")
    return text.replace(".", "p")


def _collect_refinement_paths(out_dir: Path, *, confidence_level: float | None = None) -> list[Path]:
    if confidence_level is None:
        return sorted(out_dir.glob("confidence_*/seed_*/pathway_refinement.csv"))
    confidence_dir = out_dir / f"confidence_{_slugify_confidence_level(float(confidence_level))}"
    refinement_paths = sorted(confidence_dir.glob("seed_*/pathway_refinement.csv"))
    if not refinement_paths and (confidence_dir / "pathway_refinement.csv").exists():
        refinement_paths = [confidence_dir / "pathway_refinement.csv"]
...
def _source_summary_from_refinement_paths(refinement_paths: list[Path], *, include_seed: bool = False) -> pd.DataFrame:
    ...
    for refinement_path in refinement_paths:
        ...
        if include_seed:
            confidence_label = refinement_path.parent.parent.name.replace("confidence_", "").replace("p", ".")
            seed_label = refinement_path.parent.name.replace("seed_", "")
            source_summary.insert(0, "seed", int(seed_label))
            source_summary.insert(0, "annotation_confidence", float(confidence_label))
...
def save_summary_plot(summary_df: pd.DataFrame, *, path: Path) -> None:
    save_metric_grid_plot(
        summary_df,
        x_col="annotation_confidence",
        panels=SUMMARY_PANELS,
        path=path,
        title="SLN208 prior-confidence refinement robustness",
        xlabel="Prior confidence",
    )
...
def save_summary_errorbar_plot(summary_per_run_df: pd.DataFrame, *, path: Path) -> pd.DataFrame:
    ...
    aggregated = aggregate_with_errorbars(summary_per_run_df, group_col="annotation_confidence", value_cols=value_cols)
    save_metric_grid_plot(
        aggregated,
        x_col="annotation_confidence",
        panels=SUMMARY_PANELS,
        path=path,
        title="SLN208 prior-confidence refinement robustness across seeds (mean ± SD)",
        xlabel="Prior confidence",
        errorbars=True,
    )
    return aggregated
...
def summarize_by_source(out_dir: Path, summary_df: pd.DataFrame) -> pd.DataFrame:
    rows: list[pd.DataFrame] = []
    for confidence_level in summary_df["annotation_confidence"].tolist():
        source_summary = _source_summary_from_refinement_paths(
            _collect_refinement_paths(out_dir, confidence_level=float(confidence_level))
        )
        if source_summary.empty:
            continue
        source_summary.insert(0, "annotation_confidence", float(confidence_level))
        rows.append(source_summary)
    ...
    source_df = source_df.sort_values(["source", "annotation_confidence"]).reset_index(drop=True)
...
def summarize_by_source_per_run(out_dir: Path) -> pd.DataFrame:
    ...
    if source_per_run_df.empty:
        return pd.DataFrame()
    source_per_run_df = source_per_run_df.sort_values(["source", "annotation_confidence", "seed"]).reset_index(drop=True)
...
def save_focus_source_outputs(source_df: pd.DataFrame, *, focus_source: str, out_dir: Path, plots_dir: Path) -> pd.DataFrame:
    focus_df = (
        source_df[source_df["source"] == focus_source]
        .copy()
        .sort_values("annotation_confidence")
    ...
    save_metric_grid_plot(
        ...
        title=f"SLN208 prior-confidence refinement robustness ({focus_source.upper()} only)",
        xlabel="Prior confidence",
    )
    return focus_df
...
def save_focus_source_errorbar_outputs(
    ...
    *,
    ...
    focus_per_run_df = (
        ...
        .copy()
        .sort_values(["annotation_confidence", "seed"])
        .reset_index(drop=True)
    )
    ...
    aggregated = aggregate_with_errorbars(focus_per_run_df, group_col="annotation_confidence", value_cols=value_cols)
    ...
    save_metric_grid_plot(
        ...
        title=f"SLN208 prior-confidence refinement robustness across seeds ({focus_source.upper()} only, mean ± SD)",
        xlabel="Prior confidence",
        errorbars=True,
    )
    return aggregated
...
def run_sweep(args: argparse.Namespace) -> pd.DataFrame:
    ...
    for confidence_level in args.confidence_levels:
        confidence_dir = out_dir / f"confidence_{_slugify_confidence_level(confidence_level)}"
        for seed in seeds:
            run_args = argparse.Namespace(
                data_path=args.data_path,
                out_dir=confidence_dir / f"seed_{seed:03d}",
                fpr=0.0,
                fnr=0.0,
                seed=seed,
                noise_seed=seed,
                n_uninformed_factors=args.n_uninformed_factors,
                annotation_confidence=float(confidence_level),
                model_filename=f"seed_{seed:03d}_conf{confidence_level:.3f}.h5",
            ...
            result = run(run_args)
            result["annotation_confidence"] = float(confidence_level)
            ...
            if args.save_model:
                ...
                for model_path in nested_models_dir.glob("*.h5"):
                    flat_path = flat_models_dir / run_args.model_filename
                    if flat_path.exists() or flat_path.is_symlink():
                        flat_path.unlink()
                    shutil.copy2(model_path, flat_path)

    summary_per_run_df = pd.DataFrame.from_records(summaries).sort_values(["annotation_confidence", "seed"]).reset_index(drop=True)
    summary_per_run_df.to_csv(out_dir / "summary_per_run.csv", index=False)
    summary_df = (
        summary_per_run_df.groupby("annotation_confidence", as_index=False)
        .mean(numeric_only=True)
        .sort_values("annotation_confidence")
        .reset_index(drop=True)
    )
    summary_df.to_csv(out_dir / "summary_across_confidence_levels.csv", index=False)
    save_summary_plot(summary_df, path=plots_dir / "summary_across_confidence_levels.png")
    ...
    if len(seeds) > 1:
        summary_with_errorbars_df = save_summary_errorbar_plot(
            summary_per_run_df,
            path=plots_dir / "summary_across_confidence_levels_errorbars.png",
        )
        summary_with_errorbars_df.to_csv(out_dir / "summary_across_confidence_levels_with_errorbars.csv", index=False)
    ...
    best_confidence = summary_df.loc[summary_df["mean_average_precision_true"].idxmax(), "annotation_confidence"]
    payload = {
        "data_path": str(args.data_path),
        "out_dir": str(out_dir),
        "confidence_levels": [float(x) for x in args.confidence_levels],
        "seed": int(args.seed),
        "seeds": seeds,
        ...
        "best_annotation_confidence_by_mean_ap_true": float(best_confidence),
    }
