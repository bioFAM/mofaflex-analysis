...
PYTHONPATH=. python runs/evaluate_scgraph.py \
  --data "$DATA" \
  --label-key "cell_types" \
  --batch-key "batch" \
  "${GRAPH_ARGS[@]}" \
  --out-csv outputs/sln_208_d1_scgraph.csv \
  --out-json outputs/sln_208_d1_scgraph.json

PYTHONPATH=. python runs/plot_scib_results.py \
  --input outputs/sln_208_d1_scgraph.csv \
  --metric "Corr-Weighted" \
  --out-png outputs/sln_208_d1_scgraph_corr_weighted.png \
  --title "sln_208_d1 scGraph Corr-Weighted"

echo "Done: wrote outputs/sln_208_d1_scgraph.csv/.json and outputs/sln_208_d1_scgraph_corr_weighted.png"
